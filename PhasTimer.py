    ...
    },
    {
        ...
        "map_sizes": {
            ...
            "Medium": {
                "total": 70,
                "milestones": [
                    (30, "Obambo Short ends", "Obambo Short → 30s"),
                    (20, "Normal hunt ends", "Normal → 20s"),
                    (10, "Obambo Long ends", "Obambo Long → 10s"),
                    (0, "Cursed hunt ends", "Cursed → 0s"),
                ],
            },
            "Large": {
...
class ProgressBar:
    def __init__(self, parent, color, total, milestones):
        ...
        canvas_h = BAR_H + 13
        self.cv = tk.Canvas(
            parent, width=BAR_W, height=canvas_h, bg=BG2, bd=0, highlightthickness=0
        )
        self.cv.pack(padx=16, pady=(0, 1))
        self.cv.create_rectangle(0, 0, BAR_W, BAR_H, fill=BORDER, outline="")
        self.fill_id = self.cv.create_rectangle(
            0, 0, BAR_W, BAR_H, fill=color, outline=""
        )
        for thresh, _, _ in milestones:
            ...
            self.cv.create_text(
                x, BAR_H + 6, text=lbl, fill=DIM, font=("Courier", 6), anchor="center"
            )
...
class TimerPanel:
    def __init__(self, parent, cfg, key_num):
        ...
        # Flash state
        self._flash_countdown = (
            0  # counts down 3→2→1 (integer seconds left until checkpoint)
        )
    ...
    def _build(self, parent):
        self.frame = tk.Frame(
            parent, bg=BG2, highlightbackground=BORDER, highlightthickness=1
        )
        ...
        tk.Label(
            hdr,
            text=f"{self.cfg['icon']}  {self.cfg['title']}",
            bg=BG2,
            fg=self.color,
            font=("Courier", 8, "bold"),
        ).pack(side="left")
        ...
        if "stuns" in self.cfg:
            ...
            tk.Label(
                stun_row, text="STUN", bg=BG2, fg=DIM, font=("Courier", 6, "bold")
            ).pack(side="left", padx=(0, 3))

            self.stun_disp = tk.StringVar(value="")
            tk.Label(
                stun_row,
                textvariable=self.stun_disp,
                bg=BG2,
                fg="#e84393",
                font=("Courier", 10, "bold"),
                width=4,
        ...
        self.pbar = ProgressBar(
            self.frame, self.color, self.total, self.cfg.get("milestones", [])
        )
    ...
    def _set_map_size(self, size):
        ...
        self.pbar.fill_id = self.pbar.cv.create_rectangle(
            0, 0, BAR_W, BAR_H, fill=self.color, outline=""
        )
        ...
        for s, btn in self._map_btns.items():
            active = s == size
            btn.config(
                fg=self.color if active else MUTED,
...
class PhasmoOverlay:
    ...
    def _build(self):
        ...
        tk.Label(
            tbar, text="[1] [2] [3] [4]", bg=BG3, fg=MUTED, font=("Courier", 7)
        ...
        tk.Label(
            self.root,
            text="drag title to move   •   ✕ to close",
            bg=BG,
            fg=MUTED,
            font=("Courier", 7),
        ).pack(pady=3)
    ...
    def _bind_keys(self):
        if HAS_KEYBOARD:
            ...
            self.root.after(
                500,
                lambda: print(
                    "Install 'keyboard' package for global hotkeys: pip install keyboard"
                ),
            )
